...
#### Шаг 6: второй способ — оценка ИИ по полям зрения (сделано 2026-09-24)

Решение заказчика 2026-09-24: рядом с алгоритмом — оценка «на глаз» моделью с картинкой, как это делает патолог; личные данные не передаются, только поля зрения.

- `server/cellularity_ai.py`: внутри контуров ткани выбираются поля зрения по 1 мм (грубый растр ткани 16 мкм, кандидаты с долей ткани не меньше 0,6, жадный выбор самых разнесённых, по `cellularity.ai_fields` на фрагмент, по умолчанию 6); каждое поле вырезается 768 × 768 точек (~1,3 мкм на точку) без этикеток и уходит модели с картинкой по SDK Anthropic со строгой JSON-схемой ответа (`applicable`, `cellularity_pct`, `confidence`, `note`); фрагмент — среднее по полям, где модель нашла костный мозг, стекло — среднее фрагментов с весом площади ткани. Модель — `cellularity.ai_model` в `config.yaml`, по умолчанию `claude-opus-5`. Масок и площадей у оценки нет.
- **Ключ доступа** вводит администратор в разделе «Настройки» (`PUT /api/settings/ai`), хранится в `data/ai.key` (только владелец), в журнал и ответы не попадает; альтернатива — переменная `ANTHROPIC_API_KEY` контейнера. Без ключа кнопки «Оценить ИИ» нет, запуск отвечает 400.
- Расчёты обоих способов живут в одной таблице (`cellularity_runs.method`, схема базы 8): состояние отдаёт `run` (алгоритм) и `ai_run` (ИИ); оценка ИИ идёт в потоке диспетчера (сеть, а не процессор), ход — по полям, отмена — между полями; пометка «устарел» по контурам и удаление — как у алгоритма; в CSV столбцы `method` и `fields_used`.
- Панель: под числом алгоритма второй блок «Оценка ИИ по полям зрения» с числом, по фрагментам, кнопкой «Оценить ИИ»; в «Подробнее» — список полей с оценкой и уверенностью, щелчок ведёт к полю на препарате; кнопка «Удалить оценку ИИ».
- Стоимость: около 6 полей на фрагмент, картинка ~1500 токенов, ответ короткий — по факту на рабочем сервере (4 фрагмента, 24 поля) 10–15 центов за стекло на `claude-opus-5` и 5–7 на `claude-sonnet-5`; 2026-09-24 по слову заказчика сервер переведён на `claude-sonnet-5` (`config.yaml`).
- Не сделано: отказы модели по безопасности (`stop_reason: refusal`) считаются полем без оценки; серверные запасные модели (`fallbacks`) не включены — гистологические поля их не вызывают. Сеть с сервера до `api.anthropic.com` проверена 2026-09-24 (ответ 401 без ключа).
- Проверено: тесты выбора полей и сведения без сети (`tests/cellularity_test.py`), сквозной API-тест с подменённой моделью и настройками ключа (`tests/access_test.py`), на стенде — вкладка «Настройки», кнопка появляется после ввода ключа, ложный ключ даёт понятную ошибку в панели (SDK и сеть настоящие).
- **Подача по промту заказчика (2026-09-24, после первых оценок на сервере).** Заказчик: одно число без описания при неоднородной клеточности неполно, а оценка по множеству полей всё равно ориентировочна; прислал промт с обзором и двумя участками ×20. Сделано: на фрагмент два запроса — (1) обзор фрагмента (длинная сторона до 1568 точек, контур ткани зелёным, артефакты красным) → модель выбирает два участка, неоднородных или сомнительных, точки привязываются к ближайшему окну с тканью не меньше 0,6 (`snap_regions`), при неудаче — запасной выбор по ткани; (2) обзор с пронумерованными прямоугольниками и два участка по 0,8 мм при ~0,5 мкм на точку (`DETAIL_UM`, `DETAIL_PX`) → ответ по схеме заказчика: `cellularity_percent`, `regional_min_percent`, `regional_max_percent`, `heterogeneous`, `description` (по-русски), `limitations`. Стекло — среднее фрагментов с весом площади, разброс — крайние по фрагментам, неоднородность — если её отметила модель или средние фрагментов расходятся на 10 п. п. и больше. Панель: под числом строка «клеточность неоднородная: по участкам 25–70 %» и описание модели по фрагментам без раскрытия «Подробнее», ограничения серым; в «Подробнее» — осмотренные участки с причиной выбора, щелчок ведёт к участку. CSV: столбцы `ai_min_pct`, `ai_max_pct`, `ai_heterogeneous`, `ai_description`, `ai_limitations`; `fields_used` — число участков. Настройка `cellularity.ai_fields` убрана. Цена за фрагмент: обзор дважды (~0,5–1,5 тыс. токенов каждый) и два участка по ~3,3 тыс. токенов плюс ответ с описанием — порядка того же, что 6 полей раньше.
- **Промт сокращён 2026-09-25 по слову заказчика** («Обнови промт на проде») после пробы на случае 1 (12 фрагментов, те же картинки: проценты в пределах разброса модели, неоднородных 5 из 12 вместо 12 из 12, ответ 90–120 токенов вместо 425–742, 3–6 с вместо 9–12 с, 2,1 цента за запрос оценки вместо 2,5 — цена почти целиком в картинках). Ответ модели: `cellularity_percent`, `regional_min_percent`, `regional_max_percent`, `heterogeneous` по порогу 20 п. п., `causes` из списка (жировые поля, фиброз стромы, лимфоидный узелок, очаг плотных клеток, субкортикальная зона, отёк или кровоизлияние, артефакт, другое; в коде обнуляются при однородности), `note` — одна фраза без номеров участков; поля `description`/`limitations` прежней редакции заменены на `note` (в результате по-прежнему `description`) и `causes`. Уровень усилий модели `low` (`EFFORT`), `max_tokens` 4000. Токены каждого запроса складываются в `result.usage` (запросы, вход, выход) и видны в панели и в CSV (`ai_input_tokens`, `ai_output_tokens` в строке итога). Панель: строка по фрагменту «T1: неоднородная: жировые поля. Слева ниже (~20 %), справа выше (~50 %)»; список мест ×20 — только в «Подробнее».
- **Запасной ИИ — Gemini API (решение заказчика 2026-09-25).** При исчерпании лимита у Anthropic (429, коды 402/529, «credit balance») тот же запрос с теми же картинками и промтом уходит в Gemini (`generateContent`, `responseSchema` из той же модели Pydantic — `gemini_schema`: без `$ref` и `anyOf`, «или null» → `nullable`), ответ проверяется той же схемой; если ключа Anthropic нет, а ключ Gemini есть, Gemini работает основным. Ключ — второе поле в «Настройках» (`data/gemini.key`, `PUT /api/settings/ai` с `gemini_key`) или `GEMINI_API_KEY`; модель `cellularity.gemini_model`, по умолчанию `gemini-flash-latest`. В `usage.models` — сколько запросов ответила каждая модель, в подписи расчёта — пометка про запасной. Без сторонних библиотек: запрос через `urllib`. Проверено тестами с подменённым HTTP; настоящий вызов Gemini проверяется после ввода ключа заказчиком.
