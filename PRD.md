...
| КЛ-7 | **Где и как считается.** Расчёт — в отдельном процессе с пониженным приоритетом, по одному за раз (остальные ждут в очереди), чтобы просмотр у других пользователей не тормозил, а память после расчёта сразу освобождалась. Пределы процесса — время и память (раздел «Замеры» ниже). Изображение обрабатывается плитками с полями, в памяти только плитки с тканью; пороги считаются по гистограмме всей ткани вторым проходом, поэтому результат тот же, что при расчёте целиком. Пока идёт расчёт, в панели виден ход; расчёт можно отменить |
...
1. Окраска списком (ОК-1…ОК-4) — отдельно, её можно выложить сразу.
...
- [ ] Имеющиеся сканы рабочего сервера перенесены без потерь — проверяется при выкладке
...
- [ ] Расчёт стекла с 50 мм² ткани при выбранном разрешении укладывается в 500 МБ памяти; во время расчёта тайлы у другого пользователя открываются без заметной задержки
