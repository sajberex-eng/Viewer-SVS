...
## 13. Планируется дальше

Работы, о которых заказчик попросил 2026-09-19. Подробные задания составляются отдельно, здесь зафиксирован замысел.

### 13.1. Верхняя панель вьювера: значки вместо надписей
...
Заказчик открыл сервис на iPhone: смотреть можно, но неудобно. Движение пальцем вниз тянуло страницу и сворачивало её — это исправлено сразу (страница вьювера больше не прокручивается, жесты достаются изображению). Мини-карта на телефоне теперь свёрнута, панели компактнее.

Сделано 19 сентября 2026 года: каталог на узком экране идёт в один столбец, дерево папок стоит сверху и занимает не больше трети высоты (М-3); верхняя панель сжата — имя пользователя скрыто, кнопки помещаются целиком; загрузка сканов на телефоне не показывается совсем — ни кнопки, ни списка закачек (М-5). Остальные пункты требуют отдельной работы.

| № | Замысел |
| --- | --- |
| М-1 | Верхняя панель на телефоне не помещается: видны только «Каталог» и «Выход». Решается вместе с разделом 13.1 — значки и меню «ещё» |
| М-2 | Управление увеличением на телефоне: щипок работает, но кнопки фиксированных увеличений занимают половину высоты. Возможен горизонтальный ряд снизу вместо вертикального столбца |
| М-3 | Каталог на телефоне: дерево папок и сетка карточек рассчитаны на широкий экран, нужен вид в один столбец — **сделано** |
| М-4 | Строка состояния не помещается: на телефоне оставить только увеличение |
| М-5 | Загрузка сканов с телефона не предполагается: это работа администратора за компьютером. На узком экране кнопка «Загрузить сканы» и список закачек скрыты — **сделано** |
| М-6 | Проверяется на двух браузерах, которыми пользуются в Центре: Safari на iPhone и Chrome на Android. Порядок проверки: вход, каталог, открытие скана, зум щипком, этикетка |
| М-7 | Никаких установок: адрес открывается в браузере. Значок на домашний экран добавляется средствами самого браузера, отдельное приложение не делается |
...
### 13.3. Аннотации для обучения

Заказчик назвал эту работу одной из самых полезных для обучения резидентов.

| № | Замысел |
| --- | --- |
| А-1 | Указатель на точку: стрелка или маркер на отдельной клетке с короткой подписью. Ставится там, где сейчас находится поле зрения, и хранится в координатах скана, а не экрана |
| А-2 | Контур области: обвести участок от руки или прямоугольником, подписать |
| А-3 | Аннотации видны тем, у кого есть доступ к скану; автор и время сохраняются. Кто может их создавать — преподаватель, любой пользователь или только администратор — решается при составлении задания |
| А-4 | Список аннотаций скана сбоку: щелчок переводит поле зрения к нужной. Это и есть развитие ссылки на поле зрения: вместо пересылки ссылки участок помечается прямо в скане |
| А-5 | Ссылка на поле зрения дополняется ссылкой на конкретную аннотацию |
| А-6 | Учебный сценарий: преподаватель помечает клетки, резиденты открывают скан и видят пометки; возможен режим, где пометки сначала скрыты и открываются по кнопке |

Требует решения при составлении задания: хранение (отдельная таблица в базе), права на создание и удаление, показ аннотаций в двухэкранном режиме, попадают ли подписи в журнал и не окажутся ли в них персональные данные.
