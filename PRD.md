# ТЗ: веб-сервис хранения и просмотра гистосканов SVS
...
- разместить сервис и хранилище на VPS is*hosting в Казахстане с доступом по HTTPS только для учётных записей, выданных администратором;
...
2. Администратор создаёт учётную запись, система генерирует пароль, администратор передаёт логин и пароль пользователю. При первом входе пользователь задаёт свой пароль.
...
| База | SQLite: папки, сканы, пользователи, права, загрузки, журнал. Для десятков пользователей этого достаточно |
| Кэш | Дисковый кэш готовых тайлов, лимит объёма в конфигурации (2 ГБ для диска 30 ГБ), вытеснение по давности |
| Развёртывание | Docker Compose на уже купленном VPS is*hosting (тариф Start, Ubuntu 24). HTTPS через Caddy, доменное имя |
...
| Доступ | SSH по логину и паролю (root), VNC не подключён | Первым шагом этапа 1 вход переводится на ключи, парольный вход отключается. VNC стоит запросить у провайдера как запасной вход, если SSH окажется закрыт по ошибке |
...
| Резервные копии провайдера | еженедельные | В копии попадают сканы с этикетками. Где они хранятся, нужно выяснить (раздел 14) |
...
| Окно «Доступ» | Администратору | Режим доступа, список пользователей с отметками, подпись «унаследовано от папки …» или «задано здесь» |
| Пользователи | Администратору | Таблица: логин, имя, роль, состояние, срок действия, последний вход. Действия: создать, сбросить пароль, заблокировать, удалить, «Что видит этот пользователь» |
...
| Х-13 | Незавершённые загрузки старше 3 дней удаляются автоматически: на диске 30 ГБ брошенная загрузка отнимает заметную долю места. Администратор видит их в панели загрузок и может удалить раньше |
...
| П-3 | Выдача пароля: система генерирует случайный пароль не короче 12 символов и показывает его администратору один раз с кнопкой «Скопировать». На сервере хранится только хэш (bcrypt) |
| П-4 | При первом входе и после сброса пароля пользователь обязан задать свой пароль (не короче 10 символов), чтобы администратор его не знал |
...
| Д-9 | Группы пользователей (например, «Резиденты 2026»): доступ выдаётся группе целиком (желательное требование) |
...
| Одновременных пользователей без деградации | 5 на тарифе Start (2 vCPU, 2 ГБ ОЗУ) |
...
- сервер необслуживаемый, поэтому защита настраивается нами: сетевой экран (ufw) пропускает только порты 80, 443 и SSH; вход по SSH только по ключам, парольный вход и парольный вход root отключены; перебор по SSH блокируется (fail2ban); обновления безопасности Ubuntu ставятся автоматически (unattended-upgrades);
...
- **Размещение данных.** Сервис, сканы и база находятся на VPS is*hosting в дата-центре в Алматы. Провайдер бесплатно делает еженедельные копии VPS; где они физически хранятся, нужно выяснить (раздел 14). Если за пределами Казахстана, копии провайдера отключаются, а база копируется собственными средствами на сервер в Казахстане.
...
| --- | --- | --- | --- |
| 1. Сервер на is*hosting | VPS куплен (Ubuntu 24, 2 ГБ ОЗУ, диск 30 ГБ). Проверка фактических памяти и диска; запрос VNC у провайдера как запасного входа; вход по SSH по ключу, вход по паролю и парольный вход root отключаются, отдельный пользователь для работы; сетевой экран, fail2ban, автоматические обновления; файл подкачки 2 ГБ; установка Docker с лимитом журналов; регистрация домена и запись A на адрес сервера; развёртывание готового вьювера (Docker, Caddy, HTTPS); ежедневная копия базы; замер показателей просмотра | 1 неделя | Вьювер работает по HTTPS. До конца этапа 2 на сервер загружаются только сканы без персональных данных: разграничения доступа ещё нет |
| 2. Хранилище и доступ | Дерево папок; загрузка через браузер с докачкой; роли «администратор» и «пользователь»; экран пользователей и выдача паролей; доступ к папкам и сканам с проверкой на сервере; панель этикетки; журнал с экраном; перенос существующей базы; инструкция администратора | 3–4 недели | Сервис готов к работе с реальными сканами |
...
| 4. Желательные доработки | Общий курсор в двухэкранном режиме (С-8), группы пользователей, импорт из папки SFTP, двухфакторный вход администратора, S3-бакет при росте объёма | по необходимости | — |

Критерии приёмки этапа 1:

- [ ] Сервис открывается по доменному имени по HTTPS, без авторизации доступна только страница входа
- [ ] Вход по SSH по паролю невозможен, по ключу работает; снаружи открыты только порты 80, 443 и SSH (проверка сканированием портов)
...
- [ ] Администратор создаёт пользователя, получает сгенерированный пароль; пользователь при первом входе обязан его сменить
...
- домен: `hemcenterhistodigital.kz` (на 2026-09-19 не занят). Для зоны .kz сайт должен находиться на сервере в Казахстане, это условие выполняется;
...
- [ ] Кто и у какого регистратора регистрирует `hemcenterhistodigital.kz`? Без зарегистрированного домена и записи A на адрес сервера сертификат HTTPS получить нельзя
- [ ] Где is*hosting хранит еженедельные копии VPS: в Казахстане или за рубежом? В копии попадают файлы сканов с этикетками. Вопрос задаётся в поддержку провайдера; если копии хранятся за рубежом, их нужно отключить, иначе условие «данные только в Казахстане» не выполняется
- [ ] Сколько будет пользователей и сколько из них одновременно? (заложено: до 30 учётных записей, до 5 одновременно; больше потребует докупить память)
- [ ] У кого, кроме разработчика, будет SSH-ключ от сервера?
- [ ] Обязательная смена пароля при первом входе: оставить? (заложено: да)
- [ ] Нужны ли группы пользователей уже на этапе 2? (заложено: нет, желательное требование)
