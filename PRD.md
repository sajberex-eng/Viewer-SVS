...
### 13.2. Телефон и планшет

Заказчик открыл сервис на iPhone: смотреть можно, но неудобно. Движение пальцем вниз тянуло страницу и сворачивало приложение — это исправлено сразу (страница вьювера больше не прокручивается, жесты достаются изображению). Мини-карта на телефоне теперь свёрнута, панели компактнее. Остальное требует отдельной работы.
...
| М-3 | Каталог на телефоне: дерево папок и сетка карточек рассчитаны на широкий экран, нужен вид в один столбец |
| М-4 | Строка состояния не помещается: на телефоне оставить только увеличение |
| М-5 | Загрузка сканов с телефона не предполагается: это работа администратора за компьютером |
