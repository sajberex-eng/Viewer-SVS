...
| А-15 | Удаление аннотации спрашивает подтверждение. Отмены действий (Ctrl+Z) в этом выпуске нет: она записана в 13.7 |
...
- [ ] Полигон из десятка вершин, обведённый при 20×, совпадает с той же границей ткани при 40× и после поворота
...
- [ ] Участник группы «Патологи» ставит указатель и полигон с комментарием; пользователь из группы «Резиденты» их видит, инструментов не видит и получает отказ сервера при прямом запросе на создание
- [ ] Патолог не может изменить или удалить чужую аннотацию; администратор может удалить любую
- [ ] Автор передвигает вершину своего полигона и меняет комментарий; после перезагрузки страницы правка сохранилась
- [ ] Удаление скана удаляет его аннотации
- [ ] На экране 375 точек аннотации видны, инструментов рисования нет

### 13.7. Следующий выпуск аннотаций (в этап 9 не входит)

Из документа «Предложения по инструментам аннотирования WSI» в работу сейчас не берётся перечисленное ниже. Порядок — по пользе на единицу работы, а не по порядку документа. Каждый пункт делается отдельно и только по решению заказчика.

| № | Что | Зачем и сколько стоит |
| --- | --- | --- |
| А-20 | **Классы разметки** (опухоль, некроз, строма, артефакт): справочник задаёт администратор, аннотация получает цвет и подпись по классу. Класс виден не только цветом, но и названием | Превращает пометки в данные, по которым можно фильтровать. 3–4 дня |
| А-21 | **Отмена и повтор** последнего действия (Ctrl+Z, Ctrl+Y), включая удаление | Снимает страх потерять большой контур. 2–3 дня |
| А-22 | **Стрелка** — указывает на структуру, не перекрывая её | Удобно при разборе случая. 1 день |
| А-23 | **Счётчик клеток** с категориями (положительные, отрицательные, сомнительные): щелчок ставит точку выбранной категории и увеличивает счётчик | Ускоряет полуколичественную оценку и обучение. 3–4 дня |
| А-24 | **Фигуры одной группой:** прямоугольник, эллипс, контур от руки под одной кнопкой с запоминанием последнего выбора | Быстрая грубая разметка. 3 дня |
| А-25 | **Список аннотаций с фильтрами:** по классу, автору, типу; скрытие и блокировка отдельной аннотации | Нужно, когда аннотаций десятки. 2–3 дня |
| А-26 | **Зона исключения** — участок, который не учитывается при анализе (складка, некроз, пустое стекло), с отличимым стилем | Понадобится при работе с ИИ. 2 дня |
| А-27 | **Импорт и экспорт** аннотаций в GeoJSON с координатами, классами и автором | Обмен с исследовательскими системами и ИИ. 3–4 дня |
| А-28 | **Совместное обсуждение:** ответы на аннотацию, статус «разобрано», упоминания | Это уже переписка вокруг аннотации, отдельная работа. 1–2 недели |
| А-29 | **Кисть и ластик** (растровая маска вместо контура) | **Не планируется.** Хранение и отрисовка масок на каждом уровне увеличения дороги для сервера с двумя ядрами; для обучения хватает полигона. 1,5–2 недели, если понадобится для обучения ИИ |

Измерения (линейка, площадь, периметр, угол) документ относит к обязательным, но по решению заказчика они остаются в разделе 14 «Вне рамок»: для обучения хватает линейки масштаба внизу экрана.
...
- измерения расстояний, площадей и углов: документ «Предложения по инструментам аннотирования WSI» относит их к обязательным, решением заказчика от 2026-09-20 они остаются вне рамок;
