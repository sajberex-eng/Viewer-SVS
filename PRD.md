...
| 10. Инструменты просмотра и связь сканов | Поворот на произвольный угол, отражение, рулетка, меню по правой кнопке, ползунок увеличения убирается; связь половин с учётом отражения и предложения по привязке (раздел 13.7) | 1–1,5 недели без С-10 и С-12 | По замечаниям патолога, работавшего в OneCell; решения заказчика 2026-09-20. **Не начат** |
...
### 13.7. Этап 10: инструменты просмотра и связь сканов (не начат)

**Откуда.** 2026-09-20 заказчик передал замечания патолога, который раньше работал на платформе OneCell, и попросил разобрать их критически: не нарушать визуальную простоту интерфейса, учитывать полезность и удобство. Ниже — решения заказчика после разбора. **К работам этап не приступали:** заказчик решил заняться им в другой день. Перед началом — снять открытые вопросы в конце раздела.

**Что решено не делать:** автоматическое совмещение срезов (на разных окрасках ненадёжно, серверу с 2 ГБ памяти не по силам, у OneCell работает с ошибками); третий экран (половины и так узкие); общий курсор С-8 (см. цель связи ниже); меню по правой кнопке на миниатюре скана в списке случая. Масштабная линейка внизу остаётся как есть.

| № | Требование |
| --- | --- |
| Т-1 | **Ползунок увеличения убирается** (заменяет часть Н-2): он дублирует колесо мыши. Кнопки «Весь», 2×–40× и жёлтое поле текущего увеличения остаются: калиброванное увеличение «как объектив» нужно для обучения, а в сравнении кнопки одним нажатием уравнивают обе половины. Клавиши `0`–`5`, `+`, `−` не меняются |
| Т-2 | **Поворот на произвольный угол** (расширяет Н-10): лаборанты не могут идеально ориентировать срез на стекле. Две кнопки ⟲⟳ в столбце увеличений заменяются одной кнопкой «Поворот»; она открывает небольшое окошко: ползунок угла от −180° до 180° с шагом 1°, поле с числом, кнопки «−90°», «+90°», «0°» и отметка «Отразить» (Т-3). В режиме сравнения поворот действует на активную половину: срезы на двух стёклах ориентированы по-разному. Угол, как и сейчас, попадает в ссылку на поле зрения и в связь половин. У этикетки поворот уже сделан одной кнопкой на 90° и не меняется |
...
| С-10 | **Привязка по ориентирам (предложение разработчика, обсудить с заказчиком до начала работ):** пользователь отмечает одну и ту же структуру в двух-трёх местах на обоих сканах, программа сама вычисляет сдвиг, поворот, масштаб и отражение. Результат предсказуем, в отличие от автоматики, и связь держится по всему срезу, а не только у места привязки. Растяжение ткани не исправляет ничто, поэтому нужен С-11 |
| С-11 | **Быстрая поправка связи (предложение, обсудить):** при зажатой клавише `Shift` двигается только активная половина, при отпускании связь запоминает новое взаимное положение. Сейчас для этого надо разорвать связь, поправить и связать снова |
| С-12 | **Хранение привязки пары сканов на сервере (предложение, обсудить):** чтобы резиденты открывали пару уже совмещённой. Без решения заказчика не делается: это новая таблица и права на правку |

Критерии приёмки этапа 10:

- [ ] Ползунка увеличения нет; кнопки увеличений, жёлтое поле, колесо мыши и клавиши работают как прежде
- [ ] Скан поворачивается на произвольный угол ползунком и числом, кнопки ±90° и «0°» работают; аннотации, мини-карта, линейка и настройки изображения при угле 37° остаются на месте
- [ ] Отражённый скан помечен словом «отражено»; аннотация, поставленная на отражённом скане, после снятия отражения стоит на той же клетке
- [ ] Ссылка на поле зрения передаёт угол и отражение
- [ ] Рулетка на скане с известным размером пикселя показывает длину, совпадающую с масштабной линейкой, при 4× и при 40×, при повороте и отражении; на скане без размера пикселя кнопка неактивна
- [ ] Правая кнопка мыши на препарате открывает меню; у пользователя без группы «Патологи» в нём только «Рулетка»; меню браузера вне препарата работает как обычно
- [ ] В верхней панели не прибавилось кнопок; на ширине 375 точек ничего не налезает
- [ ] Связанные половины, одна из которых отражена, идут синхронно в обе стороны
- [ ] Три набора тестов проходят

**Открытые вопросы этапа 10** (задать заказчику перед началом работ):

1. Рулетка и двойной щелчок. Заказчик заметил: «может быть, действительно проще дабл клик». Сейчас двойной щелчок приближает изображение (так задумано в разделе 6), поэтому ставить им точки рулетки нельзя без отказа от приближения. Предложение: включить рулетку (кнопка, клавиша `M` или меню по правой кнопке) и поставить две точки обычными щелчками. Уточнить, это ли имелось в виду.
2. Делать ли С-10, С-11, С-12 и в каком порядке. Предложение разработчика: Т-2 и Т-3 с С-9, затем Т-4 и Т-5, затем С-11 (дёшево), затем С-10; С-12 — только если пары сканов действительно готовят для резидентов заранее.
3. Поворот в режиме сравнения — на активную половину (как записано в Т-2) или на обе сразу, как сейчас делают кнопки ⟲⟳.
...
- **желательные доработки** (решение заказчика 2026-09-20, был этапом 10): общий курсор в режиме сравнения (С-8; 2026-09-20 заказчик подтвердил, что он не нужен: срезы разные, до клетки их не сопоставить), импорт сканов из папки SFTP, двухфакторный вход администратора, хранилище S3, регистрация домена и переход с самоподписанного сертификата на сертификат для имени.
