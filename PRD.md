...
**Следующая сессия начинается с этапа 4, пункт КД-1.**
...
- [ ] В двухэкранном режиме «Применить к обеим» меняет вид второй половины; в консоли браузера нет ошибок
- [ ] Ссылка с настройками изображения, скопированная из двухэкранного режима, открывается в другом браузере с теми же настройками в обеих половинах
- [ ] Каждая из двух половин закрывается своим крестиком — до и после «Поменять местами»
- [ ] Вход в 19:42 по Алматы показан в журнале как 19:42
- [ ] Сеть отключена на 3 минуты во время загрузки: после включения загрузка продолжается сама, без единого нажатия
- [ ] Сервер остановлен на время загрузки и запущен снова: загрузка продолжается сама
- [ ] Закрытие вкладки во время загрузки вызывает вопрос браузера
- [ ] Все три набора тестов проходят; поиск по проекту не находит удалённых имён
...
| СК-2 | После СК-1 замер повторяется. Если свежий экран на 20× по-прежнему рисуется дольше 1,5 секунды, прогрев расширяется на промежуточные уровни в области ткани. Иначе пункт закрывается без работ |

Критерии приёмки этапа 5:

- [ ] Скан загружен, через минуту открыт впервые: обзор виден целиком быстрее чем за 1 секунду; в журнале сервера нет генерации обзорных тайлов в момент открытия
- [ ] Миниатюра только что загруженного скана появляется в каталоге без задержки
- [ ] Замер проверки прав тем же способом, что в аудите: медиана не больше 1 мс
- [ ] Пользователю закрыт доступ к папке: следующий же запрос тайла из неё отвечает «не найден»; то же после блокировки пользователя. Проверки добавлены в `tests/access_test.py`
- [ ] Во время прогрева скана 1 ГБ просмотр другого скана укладывается в показатели раздела 11
...
- [ ] Chrome: загрузка файла 1 ГБ прервана закрытием браузера на 30 %; после открытия сайта — одно нажатие «Продолжить» и одно разрешение браузера, без окна выбора файла; загрузка идёт с 30 %
- [ ] То же для файла, который был перетащен мышью
- [ ] Firefox: «Продолжить» открывает выбор файла; неверный файл отклоняется с названием нужного
- [ ] Файл с тем же именем и размером, но другим содержимым к прерванной загрузке не приклеивается
- [ ] Три прерванные загрузки из разных папок продолжаются перетаскиванием трёх файлов в любую папку
- [ ] Проверки серверной части добавлены в `tests/upload_test.py`
...
- [ ] На окне шириной 1100 точек в двухэкранном режиме все действия панели доступны: значком или через меню «ещё»; ничего не обрезано
- [ ] Человек, не участвовавший в разработке, без подсказки находит: сравнение, настройку изображения, этикетку, ссылку на поле зрения
- [ ] Всеми кнопками панели можно управлять с клавиатуры; программа чтения с экрана называет каждую
- [ ] Поиск по проекту не находит слов «стёкла» и «слайд» в текстах интерфейса
- [ ] Папка перемещается в интерфейсе; ссылка на поле зрения, сохранённая до перемещения, работает
- [ ] Папка переименовывается и перемещается на планшете, касанием
- [ ] В виде «Все папки» два скана с одинаковым названием различимы по пути
