...
#### КЛ-7: память и отдельный процесс (сделано 2026-09-23, не выложено)
...
Через отдельный процесс на синтетическом SVS: пик всего процесса 307 МБ при 1 мкм (17 млн точек) и 134 МБ при 2 мкм (1 млн точек). Оценка до запуска: 150 МБ библиотек + 12 байт на точку крупнейшего фрагмента. При пределе 500 МБ фрагмент может быть до ~29 млн точек: 29 мм² при 1 мкм, 116 мм² при 2 мкм. Стекло с 50 мм² ткани при 2 мкм укладывается с запасом; при 1 мкм — только если крупнейший фрагмент не больше 29 мм² (трепанобиоптат обычно даёт фрагменты 5–20 мм²). Замер на рабочем сервере и настоящих сканах — после выкладки.
...
2. Перенос алгоритма на сервер, расчёт из командной строки (`python -m server.manage cellularity <ID>`), выгрузка контуров для QuPath; сверка с оригиналом (КЛ-11). Интерфейса ещё нет.
3. Интерфейс: контуры «ткань» и «артефакт», запуск, ход расчёта, маски, панель результата (КЛ-1…КЛ-9, КЛ-13, КЛ-14).
4. Поддержка валидации: выгрузка CSV (КЛ-12). Затем валидация у патологов Центра и выбор разрешения (КЛ-5), формулы по умолчанию и порога площади (КЛ-4).
...
- [ ] Кнопка «Клеточность» есть у сканов H&E и отсутствует у всех остальных окрасок и у скана без окраски
- [ ] На 5–10 сканах H&E результат расчёта в разрешении оригинала отличается от MarrowQuant 2.0 в QuPath по тем же контурам не больше чем на 3 процентных пункта клеточности по каждой формуле (допуск предложен разработчиком, ждёт подтверждения заказчика)
- [ ] Итог по нескольким фрагментам равен расчёту по сумме площадей
- [ ] Расчёт стекла с 50 мм² ткани при выбранном разрешении укладывается в 500 МБ памяти; во время расчёта тайлы у другого пользователя открываются без заметной задержки (по замеру 2026-09-23 на синтетике: 12 байт на точку плюс 150 МБ библиотек — при 2 мкм укладывается, при 1 мкм крупнейший фрагмент не больше 29 мм²; проверка на сервере с настоящими сканами — после выкладки)
- [ ] Расчёт работает на SVS, NDPI и KFB (проверка в образе Docker)
- [ ] Пользователь без доступа к скану не получает ни результат, ни маски (404, `tests/access_test.py`); пользователь без группы «Патологи» видит результат, но не запускает расчёт (403)
- [ ] Маски совпадают со срезом при 4× и 40×, при повороте и отражении
- [ ] Выгрузка CSV содержит все расчёты и не содержит исходных имён файлов
- [x] Перенос совпадает с ImageJ по точкам на пяти областях настоящего скана (эталон `tools/mq_reference`, 2026-09-22)
- [ ] Четыре набора тестов проходят (`tests/cellularity_test.py` — новый)
