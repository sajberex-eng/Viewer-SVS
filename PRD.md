...
| № | Требование |
| --- | --- |
| А-1 | **Указатель на клетку:** щелчок ставит стрелку, её вершина стоит ровно на отмеченной точке, а древко уходит вверх-влево и саму клетку не закрывает (решение заказчика 2026-09-20). К стрелке пишется комментарий. Хранится в координатах скана, а не экрана: остаётся на клетке при любом увеличении и повороте |
...
- **желательные доработки** (решение заказчика 2026-09-20, был этапом 10): общий курсор в режиме сравнения (С-8), импорт сканов из папки SFTP, двухфакторный вход администратора, хранилище S3, регистрация домена и переход с самоподписанного сертификата на сертификат для имени.
