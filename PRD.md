...
Версия от 2026-09-25, редакция 4 (раздел 13.8 обновлён 2026-09-25)
...
| Н-2 | Плавный зум колесом мыши с центром в точке курсора и жестом на сенсорном экране. Ползунок убран в этапе 10 (Т-1): он дублировал колесо. Диапазон: от Fit до 2× от увеличения сканирования (цифровой зум), далее зум ограничен Щипок двумя пальцами по тачпаду макбука (2026-09-26): Chrome, Edge и Firefox присылают его как колесо с Ctrl, Safari — событиями жеста; страница обрабатывает оба (`SlideView.#initTrackpadZoom`), плавно, вокруг точки под пальцами, без масштабирования страницы браузером; обычное колесо мыши работает как прежде |
...
| КЛ-5 | **Постоянное разрешение в микрометрах, а не «в 4 раза меньше»:** 1 мкм на точку (как у объектива 10×) или 2 мкм (5×) — выбор по итогам сравнения на валидации (решение заказчика 2026-09-22: выше 20× не нужно, возможно, хватит меньшего). С 2026-09-24 разрешение задаётся настройкой сервера для всех (по умолчанию 2 мкм), в панели не выбирается. Все размеры обработки заданы в микрометрах, поэтому результат не зависит от сканера (20× или 40×). Верхний предел размера жировой клетки задаётся (значение — по итогам сверки с оригиналом) |
| КЛ-6 | **Маски поверх препарата:** цвета частей как в MarrowQuant (кость — мятный, кроветворная — тёмно-синий, жир — жёлтый, строма — розовый, прочее — серый, артефакты — чёрный), подпись цветов, ползунок прозрачности, выключение одной кнопкой. Маски отдаются тайлами, как сам скан, с проверкой прав (КЛ-9). Работает при повороте и отражении (Т-2, Т-3) **Контуры «Ткань» и «Артефакт» выключаются переключателем «Показывать контуры» рядом с переключателем масок (2026-09-26):** выбор запоминается в браузере, строка видна, когда у скана есть контуры; пока выбран инструмент рисования контура, а также после «Найти контуры» и щелчка по контуру в списке они показываются в любом случае; пометки для обучения переключатель не трогает |
...
- Страница: кнопка в столбце увеличений только у H&E с размером пикселя (ОК-4, КЛ-1), панель `web/static/js/cellularity.js`: контуры (инструменты «Ткань» и «Артефакт» в слое аннотаций, список с удалением, «Найти контуры», до 2026-09-24 «Предложить контуры»), разрешение и запуск, ход по стадиям с полосой и отменой, таблица по фрагментам и итог, предупреждения, пометка «устарел», маски с выключателем и прозрачностью, подпись цветов, примечание КЛ-13. Маски — второй слой OpenSeadragon: поворот и отражение действуют сами; пересчёт координат переведён на `TiledImage`, иначе библиотека пишет в консоль предупреждение на каждый кадр. На телефоне контуров и запуска нет (КЛ-14).
...
- Замечания заказчика 2026-09-23 после первого показа (сделано и выложено, коммит `3912774`): не было видно, как убрать результат и маски и пересчитать — добавлена кнопка «Удалить расчёт» (`DELETE …/runs/{run}`, журнал `cellularity.delete`), после готового результата кнопка запуска называется «Рассчитать заново», выключатель масок и прозрачность стоят первыми в блоке результата.
- **Отзыв патолога 2026-09-24 и решения заказчика** (сделано, см. ниже): «нажал кнопку — получил результат», меньше выбора; нижняя строка состояния крадёт поле зрения; только формула 1 на виду; ИИ как второй способ оценки.

#### Шаг 5: по отзыву патолога (сделано 2026-09-24)

- **Фоновый расчёт после загрузки.** Для скана H&E с размером пикселя сервер сам находит контуры по миниатюре и считает клеточность в фоне сразу после загрузки (и после смены окраски на H&E), с пониженным приоритетом; журнал помечает такой запуск «автоматически после загрузки». Патолог открывает скан с готовым результатом. Настройка `cellularity.auto` (по умолчанию включено).
- **Разрешение задаётся на сервере** одно для всех (`cellularity.resolution`, по умолчанию `2` — 2 мкм на точку, как оригинал считает сканы 20×, вчетверо быстрее 1 мкм); выбора в панели нет. Команда `manage cellularity` сохраняет `--resolution` для сверок. Валидация может изменить значение.
- **Панель в два слоя.** Сверху одно число — клеточность по формуле 1 итогом по стеклу, строкой по фрагментам (T1 40 %, T2 64 %), предупреждения, пометка «устарел», выключатель масок, одна кнопка «Рассчитать» (контуры найдутся сами, если их нет; после результата — «Рассчитать заново»). Всё остальное — контуры, площади, формула 2, прозрачность, подпись цветов, удаление расчёта, выгрузка — под сворачиваемым «Подробнее».
- **Нижней строки состояния нет.** Увеличение (жёлтое поле) и пометка «отражено» — в шапке справа; имя пользователя — кружок с инициалами и подсказкой; «Выход» — значок; сообщения и подсказки всплывают над препаратом на 6 с и исчезают, на телефоне только ошибки. Поле зрения выросло на 26 точек по высоте.
- Проверено на стенде в браузере: одна кнопка на скане без контуров, ход, результат, «Подробнее», всплывашка, телефон. Тест прав: фоновый запуск завершается при разрешении из настроек, повторно при свежем результате не стартует, помечен в журнале; запуск одной кнопкой.
#### Шаг 6: второй способ — оценка ИИ «на глаз» (сделано 2026-09-24…25, состояние на 2026-09-25)

Решение заказчика 2026-09-24: рядом с алгоритмом — оценка «на глаз» моделью с картинкой, как это делает патолог; личные данные не передаются, только изображения препарата без этикеток. Решения 2026-09-25: промт сокращён; добавлен второй поставщик ИИ (Gemini); основной поставщик выбирается переключателем, по умолчанию **Gemini**.

**Как считается** (`server/cellularity_ai.py`, на каждый фрагмент ткани два запроса):

1. обзор фрагмента (длинная сторона до 1568 точек) с зелёным контуром ткани и красными контурами артефактов → модель выбирает два неоднородных или сомнительных участка (центр в пикселях обзора и причина не больше шести слов); точки привязываются к ближайшему окну с долей ткани не меньше 0,6 (`snap_regions`), при неудаче — запасной выбор по ткани;
2. тот же обзор с пронумерованными прямоугольниками и два участка по 0,8 мм при ~0,5 мкм на точку (`DETAIL_UM`, `DETAIL_PX`, объектив ×20) → ответ по схеме: `cellularity_percent`, `regional_min_percent`, `regional_max_percent`, `heterogeneous` (порог 20 п. п. на заметной площади), `causes` из списка (жировые поля, фиброз стромы, лимфоидный узелок, очаг плотных клеток, субкортикальная зона, отёк или кровоизлияние, артефакт, другое; при однородности обнуляются), `note` — одна фраза без номеров участков.

Стекло — среднее фрагментов с весом площади ткани, разброс — крайние значения, неоднородно, если таков хотя бы один фрагмент. Масок и площадей у оценки нет. Промт (`SYSTEM_PROMPT`, `PICK_PROMPT`, `ESTIMATE_PROMPT`) — текст заказчика, сокращённый 2026-09-25 по его слову после пробы на случае 1: менять только по его слову. Уровень усилий модели Anthropic `low`, `max_tokens` 4000.

**Два поставщика.** Gemini API (модель `cellularity.gemini_model`, по умолчанию `gemini-flash-latest`; `generateContent`, `responseSchema` строится из той же модели Pydantic функцией `gemini_schema`: без `$ref` и `anyOf`, «или null» → `nullable`; запросы через `urllib`, без сторонних библиотек) и Anthropic (SDK, модель `cellularity.ai_model`, на сервере `claude-sonnet-5`). Ответ любого проверяется одной и той же схемой Pydantic.

- **Основной ИИ** выбирает администратор в «Настройках» (список «Основной ИИ», `PUT /api/settings/ai` с `provider`, файл `data/ai.provider`, запись в журнале); по умолчанию — Gemini (`cellularity.ai_provider`). Выбор в «Настройках» главнее настройки.
- **Запасной — второй поставщик.** Если у основного исчерпан лимит (Anthropic: 429, 402, 529, «credit balance»; Gemini: 429 или квота), тот же запрос с теми же картинками и промтом уходит ко второму, если задан его ключ; иначе оценка останавливается с понятным сообщением. Если ключа основного нет, оценка идёт через второй.
- **Ключи** вводит администратор в «Настройках» (`data/ai.key` для Anthropic и `data/gemini.key` для Gemini, только владелец; либо переменные `ANTHROPIC_API_KEY`, `GEMINI_API_KEY` контейнера); наружу и в журнал не попадают. Без единого ключа кнопки «Оценить ИИ» нет, запуск отвечает 400.
- В результате `usage`: число запросов, токены, `models` — сколько запросов ответила каждая модель. Подпись расчёта называет модель, ответившую больше всех, и при переходе на запасной — остальные.

**Интерфейс.** Под числом алгоритма второй блок «Оценка ИИ»: число крупно, разброс по частям, строка по каждому фрагменту («T2: неоднородная: лимфоидный узелок, жировые поля. Участок с узелком плотнее, края более жировые»); кнопки «Оценить ИИ», «Отменить», «Удалить оценку ИИ»; в «Подробнее» — места, осмотренные при ×20 (щелчок ведёт к месту на препарате), модель, число запросов и токенов. Ход показывается по фрагментам и шагам («выбор участков», «оценка»). Расчёты обоих способов живут в одной таблице (`cellularity_runs.method`, схема базы 8), оценка ИИ идёт в потоке диспетчера (сеть, а не процессор), отмена — между запросами. CSV: `method`, `fields_used` (число участков), `ai_min_pct`, `ai_max_pct`, `ai_heterogeneous`, `ai_causes`, `ai_note`, а в строке итога `ai_input_tokens`, `ai_output_tokens`.

**Стоимость** (замеры на сканах случая 1, тарифы на 2026-09-25): почти всю цену составляют картинки — участок ×20 около 3,3 тыс. токенов.

| Поставщик и модель | Запросов на стекло из 4 фрагментов | Токенов | Ориентировочная цена |
| --- | --- | --- | --- |
| Gemini `gemini-flash-latest` | 8 | 21 тыс. вход + 7,6 тыс. выход (в выход входит размышление) | около 2,5 цента |
| Anthropic `claude-sonnet-5`, промт от 2026-09-25 | 8 | около 47 тыс. вход + 0,7 тыс. выход | около 10 центов |
| Anthropic `claude-opus-5` | 8 | те же | около 25 центов |

Цена Gemini посчитана по тарифу Flash 0,30 $ за млн токенов на вход и 2,50 $ на выход (справочные значения, сверять с консолью Google), Anthropic — по тарифам Sonnet 5 (2 $ и 10 $) и Opus 5 (5 $ и 25 $); токены Anthropic взяты из пробы на 12 фрагментах (9,5–12 тыс. на запрос оценки, около 2 тыс. на выбор участков), у Opus цена расчётная, замера не было.

**Проверено.** Тесты без сети (`tests/cellularity_test.py`: привязка участков, сведение, схема Gemini, порядок и переход между поставщиками, выбор основного) и сквозной API-тест с подменённой моделью и настройками (`tests/access_test.py`); на стенде — вкладка «Настройки», переключатель, ключи; на рабочем сервере обе схемы ответа приняты настоящим API Anthropic, а полный ход оценки на скане 02 случая 1 (4 фрагмента, 61 с) выполнен через Gemini: стекло 60 % при 58 % у алгоритма и 52 % у Anthropic. Проба промта на 12 фрагментах случая 1 (те же картинки): проценты в пределах разброса модели, неоднородных 5 из 12 вместо 12 из 12, ответ 90–120 токенов вместо 425–742, время запроса 3–6 с вместо 9–12 с.

**Не сделано.** Отказы модели по безопасности (`stop_reason: refusal`) считаются фрагментом без оценки, серверные `fallbacks` Anthropic не включены. Вопрос «какой ИИ ближе к глазу патолога» решает валидация у патологов Центра.
