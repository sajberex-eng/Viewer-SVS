...
каждые четверть секунды, на Linux ещё RLIMIT_DATA как запасной предел внутри процесса;
...
def _limit_data_segment(limit_mb: float) -> None:
    """Запасной предел внутри процесса: RLIMIT_DATA (куча и анонимные отображения — то, что
    выделяет NumPy). Он не считает библиотеки и потому мягче наблюдателя за RSS."""
    try:
        import resource
        limit = int(limit_mb * 2 ** 20)
